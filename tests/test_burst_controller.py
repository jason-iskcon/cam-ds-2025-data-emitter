...
class TestBurstController:
    ...
    def test_init_zero_base_rate(self):
        ...
        with pytest.raises(AssertionError, match="base_rate must be positive"):
            BurstController(probability=0.05, multiplier=5.0, duration_events=10, base_rate=0.0)

    def test_init_negative_base_rate(self):
        """Test with negative base rate raises validation error."""
        with pytest.raises(AssertionError, match="base_rate must be positive"):
            BurstController(probability=0.05, multiplier=5.0, duration_events=10, base_rate=-1.0)
    ...
    def test_init_invalid_probability(self):
        ...
        with pytest.raises(AssertionError, match="probability must be in"):
            BurstController(probability=-0.1, multiplier=5.0, duration_events=10, base_rate=10.0)
        with pytest.raises(AssertionError, match="probability must be in"):
            BurstController(probability=1.5, multiplier=5.0, duration_events=10, base_rate=10.0)
    ...
    def test_init_invalid_multiplier(self):
        ...
        with pytest.raises(AssertionError, match="multiplier must be positive"):
            BurstController(probability=0.05, multiplier=0.0, duration_events=10, base_rate=10.0)
        with pytest.raises(AssertionError, match="multiplier must be positive"):
            BurstController(probability=0.05, multiplier=-1.0, duration_events=10, base_rate=10.0)
    ...
    def test_init_invalid_duration(self):
        ...
        with pytest.raises(AssertionError, match="duration_events must be positive"):
            BurstController(probability=0.05, multiplier=5.0, duration_events=0, base_rate=10.0)
        with pytest.raises(AssertionError, match="duration_events must be positive"):
            BurstController(probability=0.05, multiplier=5.0, duration_events=-1, base_rate=10.0)
    ...
    def test_should_start_burst_probability_zero_short_circuit(self):
        ...
        with patch(
            "emitter._streaming.random.random", side_effect=AssertionError("Should not be called")
        ):
            assert controller.should_start_burst() is False
