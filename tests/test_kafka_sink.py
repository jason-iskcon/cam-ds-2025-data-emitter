...
class TestKafkaSink:
    ...
    def test_init_all_acks_modes(self, kafka_mock):
        ...
        for acks in ("0", "1", "all"):
            ...
            sink = KafkaSink(bootstrap="localhost:9092", topic="transactions", acks=acks)
    ...
    def test_init_with_enum(self, kafka_mock):
        ...
        for acks_enum in KafkaAcks:
            # Reset mock call count for each iteration
            mock_producer_class.reset_mock()
            sink = KafkaSink(bootstrap="localhost:9092", topic="transactions", acks=acks_enum)
    ...
    def test_init_invalid_acks_raises_error(self, kafka_mock):
        ...
        with pytest.raises(ValueError, match="acks must be one of"):
            KafkaSink(bootstrap="localhost:9092", topic="transactions", acks="invalid")

        with pytest.raises(ValueError, match="acks must be one of"):
            KafkaSink(bootstrap="localhost:9092", topic="transactions", acks="2")
